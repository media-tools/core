	...
	internal class TarHeader : ITarHeader
	{
		...
		public virtual bool UpdateHeaderFromBytes ()
		{
			...
			if ((buffer [124] & 0x80) == 0x80) { // if size in binary
				...
				SizeInBytes = Core.Net.IPAddress.NetworkToHostOrder (sizeBigEndian);
			} else {
				SizeInBytes = Convert.ToInt64 (Encoding.UTF8.GetString (buffer, 124, 11), 8);
			}
			long unixTimeStamp = Convert.ToInt64 (Encoding.UTF8.GetString (buffer, 136, 11), 8);
			LastModification = TheEpoch.AddSeconds (unixTimeStamp);

			var storedChecksum = Convert.ToInt32 (Encoding.UTF8.GetString (buffer, 148, 6));
